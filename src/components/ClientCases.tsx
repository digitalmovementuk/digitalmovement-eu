...
import { caseStudies, casesIntro } from "../content";
...
export function ClientCases() {
  ...
  return (
    <section
      ...
      id="cases"
      data-surface="light"
      className="surface-light relative overflow-hidden md:min-h-[200vh] pt-28 sm:pt-32 md:pt-36 pb-20 sm:pb-28 md:pb-36"
    >
      ...
      <div className="relative container-v3">
        ...
        <div className="grid lg:grid-cols-[1.2fr_1fr] gap-6 lg:gap-12 items-end text-center lg:text-left">
          <div>
            <Reveal>
              <p className="eyebrow text-ink-muted">{casesIntro.eyebrow}</p>
            </Reveal>
            <Reveal delay={0.05}>
              <h2
              ...
              >
                {casesIntro.headlineMain}
                <span className="block text-ink/55">{casesIntro.headlineSub}</span>
              </h2>
            </Reveal>
          </div>
          <Reveal delay={0.12}>
      ...
      >
        ...
        <div className="p-7 sm:p-10 md:p-12 lg:p-14 flex flex-col justify-between min-h-[360px] md:min-h-[440px] text-ink">
          <div>
            ...
            <h3
              className="mt-2.5 max-w-[24ch] balance text-ink"
              style={{
          ...
          {/* Der Beleg zur Referenz: die Seite des Kunden selbst.
              rel="noopener" ist Pflicht, sonst kann die geöffnete Seite
              über window.opener auf diese hier zugreifen. */}
          {study.url ? (
            <a
              href={study.url}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-6 inline-flex items-center gap-1.5 text-[14px] font-semibold text-ink hover:text-[#EC178D] underline underline-offset-4 decoration-ink/25 hover:decoration-[#EC178D] transition-colors"
            >
              {casesIntro.visitLabel}
